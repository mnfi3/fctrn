...
! function($) {
  function SignaturePad(selector, options) {
    ...
    function clearMouseLeaveTimeout() {
      clearTimeout(mouseLeaveTimeout), mouseButtonDown = mouseLeaveTimeout = !1
    }

    function drawLine(e, newYOffset) {
      var offset, newX, newY;
      if (e.preventDefault(), offset = $(e.target).offset(), clearTimeout(mouseLeaveTimeout), mouseLeaveTimeout = !1, newY = void 0 !== e.targetTouches ? (newX = Math.floor(e.targetTouches[0].pageX - offset.left), Math.floor(e.targetTouches[0].pageY - offset.top)) : (newX = Math.floor(e.pageX - offset.left), Math.floor(e.pageY - offset.top)), previous.x === newX && previous.y === newY) return !0;
      null === previous.x && (previous.x = newX), null === previous.y && (previous.y = newY), newYOffset && (newY += newYOffset), canvasContext.beginPath(), canvasContext.moveTo(previous.x, previous.y), canvasContext.lineTo(newX, newY), canvasContext.lineCap = settings.penCap, canvasContext.stroke(), canvasContext.closePath(), output.push({
        lx: newX,
        ly: newY,
        mx: previous.x,
        my: previous.y
      }), previous.x = newX, previous.y = newY, settings.onDraw && "function" == typeof settings.onDraw && settings.onDraw.apply(self)
    }
    ...
    function stopDrawing(e) {
      ...
      }) : canvas.unbind("mousemove.signaturepad"), 0 < output.length && settings.onDrawEnd && "function" == typeof settings.onDrawEnd && settings.onDrawEnd.apply(self)), previous.x = null, previous.y = null, settings.output && 0 < output.length && $(settings.output, context).val(JSON.stringify(output))
    }

    function clearCanvas() {
      canvasContext.clearRect(0, 0, element.width, element.height), canvasContext.fillStyle = settings.bgColour, canvasContext.fillRect(0, 0, element.width, element.height), settings.displayOnly || function() {
        if (!settings.lineWidth) return;
        canvasContext.beginPath(), canvasContext.lineWidth = settings.lineWidth, canvasContext.strokeStyle = settings.lineColour, canvasContext.moveTo(settings.lineMargin, settings.lineTop), canvasContext.lineTo(element.width - settings.lineMargin, settings.lineTop), canvasContext.stroke(), canvasContext.closePath()
      }(), canvasContext.lineWidth = settings.penWidth, canvasContext.strokeStyle = settings.penColour, $(settings.output, context).val(""), output = [], stopDrawing()
    }
    ...
    function initDrawEvents(e) {
      if (eventsBound) return !1;
      eventsBound = !0, $("input").blur(), void 0 !== e.targetTouches && (touchable = !0), touchable ? (canvas.each(function() {
    ...
    function drawIt() {
      $(settings.typed, context).hide(), clearCanvas(), canvas.each(function() {
        ...
        if (e.preventDefault(), 1 < e.which) return !1;
        mouseButtonDown = !0, initDrawEvents(e), startDrawing(e)
    ...
    function typeIt() {
      clearCanvas(), eventsBound = !1, canvas.each(function() {
      ...
      }), $(document).unbind("mouseup.signaturepad"), canvas.unbind("mousedown.signaturepad"), canvas.unbind("mousemove.signaturepad"), canvas.unbind("mouseleave.signaturepad"), $(settings.clear, context).unbind("click.signaturepad"), $(settings.typed, context).show(), $(settings.drawIt, context).bind("click.signaturepad", function(e) {
    ...
    function type(val) {
      ...
      if (typeItNumChars = cleanedVal.length, typed.html(cleanedVal), cleanedVal) {
        if (oldLength < typeItNumChars && typed.outerWidth() > element.width)
        ...
        if (typeItNumChars < oldLength && typed.outerWidth() + edgeOffset < element.width && typeItCurrentFontSize < typeItDefaultFontSize)
          for (; typed.outerWidth() + edgeOffset < element.width && typeItCurrentFontSize < typeItDefaultFontSize;) typeItCurrentFontSize++, typed.css("font-size", typeItCurrentFontSize + "px")
      } else typed.css("font-size", typeItDefaultFontSize + "px")
    }
    ...
    function validateForm() {
      ...
      return settings.onBeforeValidate && "function" == typeof settings.onBeforeValidate ? settings.onBeforeValidate.apply(self, onBeforeArguments) : function(context, settings) {
      ...
      }.apply(self, onBeforeArguments), settings.drawOnly && output.length < 1 && (valid = !(errors.drawInvalid = !0)), "" === $(settings.name, context).val() && (valid = !(errors.nameInvalid = !0)), settings.onFormError && "function" == typeof settings.onFormError ? settings.onFormError.apply(self, onErrorArguments) : function(errors, context, settings) {
      ...
      }.apply(self, onErrorArguments), valid
    }

    function drawSignature(paths, context, saveOutput) {
      for (var i in paths) "object" == typeof paths[i] && (context.beginPath(), context.moveTo(paths[i].mx, paths[i].my), context.lineTo(paths[i].lx, paths[i].ly), context.lineCap = settings.penCap, context.stroke(), context.closePath(), saveOutput && paths[i].lx && output.push({
        lx: paths[i].lx,
        ly: paths[i].ly,
        mx: paths[i].mx,
        my: paths[i].my
    ...
    $.extend(self, {
      ...
      regenerate: function(paths) {
        self.clearCanvas(), $(settings.typed, context).hide(), "string" == typeof paths && (paths = JSON.parse(paths)), drawSignature(paths, canvasContext, !0), settings.output && 0 < $(settings.output, context).length && $(settings.output, context).val(JSON.stringify(output))
      },
      ...
      getSignatureImage: function() {
        var data, tmpCanvas = document.createElement("canvas"),
          tmpContext = null;
  ...
  $.fn.signaturePad = function(options) {
    ...
    return this.each(function() {
      $.data(this, "plugin-signaturePad") ? (api = $.data(this, "plugin-signaturePad")).updateOptions(options) : ((api = new SignaturePad(this, options)).init(), $.data(this, "plugin-signaturePad", api))
    }), api
